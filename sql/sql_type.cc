...
decimal_part_t Type_numeric_attributes::find_max_decimals(Item **item, uint nitems)
{
  decimal_part_t res= 0;
...
decimal_part_t Type_numeric_attributes::
find_max_decimal_int_part(Item **item, uint nitems)
{
  decimal_part_t max_int_part= 0;
  for (uint i=0 ; i < nitems ; i++)
...
Type_numeric_attributes::aggregate_numeric_attributes_decimal(Item **item,
                                                              ...
                                                              bool unsigned_arg)
{
  uint32 max_int_part= find_max_decimal_int_part(item, nitems);
  decimals= find_max_decimals(item, nitems);
  uint32 precision= MY_MIN(max_int_part + decimals, DECIMAL_MAX_PRECISION);
...
decimal_part_t Type_handler::Item_time_precision(THD *thd, Item *item) const
{
  return MY_MIN(item->decimals, TIME_SECOND_PART_DIGITS);
}


decimal_part_t Type_handler::Item_datetime_precision(THD *thd, Item *item) const
{
  return MY_MIN(item->decimals, TIME_SECOND_PART_DIGITS);
}


decimal_part_t Type_handler_string_result::
  ...
  if ((tmp= item->val_str(&buf)) &&
      (is_time ?
       ...
       Datetime(thd, &status, tmp->ptr(), tmp->length(), tmp->charset(),
         ...
         is_valid_datetime()))
    return (decimal_part_t) MY_MIN(status.precision, TIME_SECOND_PART_DIGITS);
  return (decimal_part_t) MY_MIN(item->decimals, TIME_SECOND_PART_DIGITS);
}

/***************************************************************************/

decimal_part_t Type_handler::Item_decimal_scale(const Item *item) const
{
  return (item->decimals < NOT_FIXED_DEC ?
          item->decimals :
          (decimal_part_t) MY_MIN(item->max_length, DECIMAL_MAX_SCALE));
}

decimal_part_t Type_handler_temporal_result::
...
decimal_part_t Type_handler::Item_divisor_precision_increment(const Item *item) const
{
  return item->decimals;
}
...
decimal_part_t Type_handler_string_result::Item_decimal_precision(const Item *item) const
{
  ...
  */
  return res ? (decimal_part_t) MY_MIN(res, DECIMAL_MAX_PRECISION) : (decimal_part_t) 1;
}

decimal_part_t Type_handler_real_result::Item_decimal_precision(const Item *item) const
{
  uint res= item->max_char_length();
  return res ? (decimal_part_t) MY_MIN(res, DECIMAL_MAX_PRECISION) : (decimal_part_t) 1;
}

decimal_part_t Type_handler_decimal_result::Item_decimal_precision(const Item *item) const
{
  uint prec= my_decimal_length_to_precision(item->max_char_length(),
                                            item->decimals,
                                            item->unsigned_flag);
  return (decimal_part_t) MY_MIN(prec, DECIMAL_MAX_PRECISION);
}

decimal_part_t Type_handler_int_result::Item_decimal_precision(const Item *item) const
{
 uint prec= my_decimal_length_to_precision(item->max_char_length(),
                                           item->decimals,
                                           item->unsigned_flag);
...
decimal_part_t Type_handler_time_common::Item_decimal_precision(const Item *item) const
{
  return (decimal_part_t) (7 + MY_MIN(item->decimals, TIME_SECOND_PART_DIGITS));
}

decimal_part_t Type_handler_date_common::Item_decimal_precision(const Item *item) const
{
  return 8;
}

decimal_part_t Type_handler_datetime_common::
Item_decimal_precision(const Item *item) const
{
  return (decimal_part_t) (14 + MY_MIN(item->decimals, TIME_SECOND_PART_DIGITS));
}

decimal_part_t Type_handler_timestamp_common::
Item_decimal_precision(const Item *item) const
{
  return (decimal_part_t) (14 + MY_MIN(item->decimals, TIME_SECOND_PART_DIGITS));
}
...
bool get_length_and_scale(ulonglong length, ulonglong decimals,
                          uint *out_length, decimal_part_t *out_decimals,
                          ...
                          Item *a)
{
  ...
  *out_decimals=  (decimal_part_t)  decimals;
